...
**Command:**
```
search_replace file_path="app/(app)/dashboard/page.tsx" old_string="const baseUrl = process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000';" new_string="const baseUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://costshield.cloud';"
```
...
- Production URLs: **FIXED** (localhost → costshield.cloud)
...
4. ✅ **Localhost Hardcode:** Changed fallback from `localhost:3000` to `https://costshield.cloud`
